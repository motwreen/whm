...
{	
	...
	{
        $host               = Config::get('whm.host');
		$port 				= Config::get('whm.port');
		$endpoint 			= Config::get('whm.endpoint');
		$url 				= "https://$host:$port/$endpoint/$this->method?api.version=1$this->params";
		$this->url 			= $url;
	}
	...
	{
		$username	= Config::get('whm.username');
		$password	= Config::get('whm.password');
